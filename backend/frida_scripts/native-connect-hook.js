/*
 * VENDORED from HTTP Toolkit's frida-interception-and-unpinning project.
 * Do not edit the body below EXCEPT the clearly-marked "LOCAL PATCH" blocks —
 * update by re-vendoring from upstream, then re-applying those patches. The only
 * local change is a mitmproxy-compat fix in performSocksHandshake (unwrap
 * IPv4-mapped IPv6 targets to IPv4, else mitmproxy SOCKS5 replies "host
 * unreachable"). The config globals it reads (PROXY_HOST,
 * PROXY_PORT, PROXY_SUPPORTS_SOCKS5, IGNORED_NON_HTTP_PORTS, BLOCK_HTTP3,
 * DEBUG_MODE) are generated by FridaController._build_script and prepended ahead
 * of this file. For us PROXY_PORT is the mitmproxy SOCKS5 listener (config.SOCKS_PORT,
 * 51081) and PROXY_SUPPORTS_SOCKS5 is true. See docs/frida.md and NOTICE.md.
 *
 * This file is AGPL-3.0-or-later (see header below); bundling it makes the
 * distributed app AGPL-3.0-or-later. See the repo LICENSE/README note.
 */
...
(() => {
    ...
    function performSocksHandshake(sockfd, targetHostBytes, targetPort, isIPv6) {
        ...
        // --- LOCAL PATCH (mitmproxy compat, deviates from upstream verbatim) ---
        // Android apps routinely use AF_INET6 sockets with IPv4-mapped addresses
        // (::ffff:a.b.c.d) even for IPv4 destinations. mitmproxy's SOCKS5 server
        // can't reach those when they arrive as ATYP=IPv6 — it replies 4 ("host
        // unreachable") and the app loses all connectivity. HTTP Toolkit's own
        // proxy unwraps these server-side; mitmproxy doesn't, so unwrap here to a
        // real IPv4 (ATYP=1). Genuine IPv6 destinations still go out as ATYP=6.
        const isV4Mapped = isIPv6 && targetHostBytes.length === 16 &&
            targetHostBytes.slice(0, 10).every(b => b === 0) &&
            targetHostBytes[10] === 0xff && targetHostBytes[11] === 0xff;

        if (isV4Mapped) {
            req.push(0x01); // ATYP: IPv4 (unwrapped from ::ffff:)
            req.push(...targetHostBytes.slice(12));
        } else if (isIPv6) {
            req.push(0x04); // ATYP: IPv6
            req.push(...targetHostBytes);
        } else {
        ...
        req.push((targetPort >> 8) & 0xff, targetPort & 0xff);
