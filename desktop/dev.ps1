...
# spawns the Python backend and loads Vite (:5173) for hot-reload, so this single
...
try {
    ...
    Write-Host "[dev] starting Vite dev server (:5173)..." -ForegroundColor Cyan
    $vite = Start-Process -FilePath "npm.cmd" `
        -ArgumentList @("--prefix", "$root\frontend", "run", "dev") `
        -PassThru -WindowStyle Hidden

    if (-not (Wait-Port 5173 30)) { throw "Vite did not come up on :5173 within 30s" }
    Write-Host "[dev] Vite up. Launching Electron (it spawns the Python backend on :8770/:8080)..." -ForegroundColor Green
    ...
    foreach ($c in (Get-NetTCPConnection -LocalPort 5173 -State Listen -ErrorAction SilentlyContinue)) {
